...
void Scene::Update(float deltaTs, Input* input)
{
	...
	if (_simulation_start == true)
	{
		...
		if (pos.y > _level->GetPosition().y+0.4)
		{
			_displace.y = (((_v_i.y+(_v_i.y+(_gravity*deltaTs)))/2)*deltaTs);
			// Before I changed it: pos += glm::vec3(0.0, -deltaTs, 0.0);
			pos += _displace;
			_v_i.y = _v_i.y + (_gravity*deltaTs);
